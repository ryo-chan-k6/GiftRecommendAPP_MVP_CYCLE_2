...
flowchart LR
  ...
  subgraph DB["PostgreSQL (Supabase)"]
    ...
    APL["apl schema<br/>(item / item_tag / genre / tag / tag_group / shop / item_rank_snapshot / item_embedding_source / item_embedding)"]
  end

  %% ========== Components ==========
  subgraph Lib["共通部品（モジュール）"]
    RC["rakuten_client<br/>(API呼び出し/レート制御)"]
    OC["openai_client<br/>(Embeddings API)"]
    ...
    AR["apl_repos<br/>(upsert item/genre/tag/shop,<br/>upsert item_tag,<br/>update is_active,<br/>upsert snapshot,<br/>upsert embedding_source/embedding)"]
  end
...
flowchart TD
  ...
  T --> A
  A --> ES["JOB-E-01 Embedding Source Build<br/>apl.item_embedding_source"]
  ES --> EB["JOB-E-02 Embedding Build<br/>OpenAI → apl.item_embedding"]
  EB --> END((END))
