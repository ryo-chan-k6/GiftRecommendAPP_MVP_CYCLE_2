...
### 8.1 統一CI（ci.yaml）
- トリガー：`push`（main 以外）/ `pull_request`
- 対象：batch / web / api / reco を1ワークフローで並列検証
- ワークフロー：`.github/workflows/ci.yaml`
- バッチ単体テスト：`batch-unit` ジョブで `pytest -m "unit"` を実行

### 8.2 CIで実行するジョブ（batch-unit）
- Unit テスト
  - `pytest -m "unit"`
- （将来）Lint / Format
...
必要なら、この方針書に合わせて統一CI（`ci.yaml`）は `batch-unit` ジョブで `pytest -m "unit"` を実行し、本方針書の Unit テストを担保する。`pytest.ini` / `pyproject.toml`（ruff/black採用時）まで “コピペで動く形” で一式出します。
