...
- 入力： apl.item の当日バッチ処理 upsert 実施レコードから抽出した `rakuten_genre_id` <br>※`apl.staging` の `(source='rakuten', entity='genre')` で、今回ジョブが更新した `saved_at` 以降の source_id を入力集合とする
- 楽天 Genre API を呼び出す
- raw JSON を S3 に保存（hash差分）
...
- 入力： apl.item_tag の当日バッチ処理 upsert 実施レコードから抽出した `rakuten_tag_id` <br>※`apl.staging` の `(source='rakuten', entity='tag')` で、今回ジョブが更新した `saved_at` 以降の source_id を入力集合とする
- 楽天 Tag API を呼び出す
- raw JSON を S3 に保存（hash差分）
